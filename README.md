...
- **コンポーネント間連携 (`zsh-functions/`)**:
  `components/dotfiles-*/zsh-functions/` 配下のシェル関数ファイルを `zshrc` が自動検出して動的に読み込みます。
- **予測補完 & fzf インテグレーション**:
  `zsh-autosuggestions` による即時予測入力と、`fzf` による2枠浮動カード（Floating Window）形式のリッチな `Ctrl + R` 履歴検索 TUI が利用可能です。
- **プロンプトテーマ**:
  `starship/starship.toml` による軽量かつカスタマイズされた Starship プロンプトを採用しています。
