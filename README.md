...
### ⚠️ 単体使用時の注意点
本リポジトリは `dotfiles-core` の共通 Makefile ルール（`common-mk`）に依存しています。単体で使用（クロー ン）する場合は、以下の手順が必要です：
...
特に、**`config/config.example.zsh` から `config/config.zsh` を作成**し、**`.zsh_secrets` を用意**することが必須です。
